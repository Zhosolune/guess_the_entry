...
### 2025-11-11 顶部栏新增设置按钮

- 目标：在顶部栏主题切换按钮旁新增设置图标按钮，样式与现有图标按钮一致，位置紧挨主题按钮。
- 修改内容：
...
- 目标：修复初始页内容被固定 TopBar 遮挡的问题。
- 修改内容：App 初始页容器添加 `mt-[var(--topbar-h)]`，与 TopBar 高度一致，确保首屏内容正常可见。
- 验证：在 `http://localhost:5173/` 预览，初始页内容位于 TopBar 下方居中显示，无重叠。
### 2025-11-11 初始页选择区域扩宽
